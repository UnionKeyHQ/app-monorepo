package so.unionkey.app.wallet.reactModule
...
import so.unionkey.app.wallet.nfc.NFCExceptions
import so.unionkey.app.wallet.nfc.NfcUtils
import so.unionkey.app.wallet.nfc.broadcast.NfcStatusChangeBroadcastReceiver
import so.unionkey.app.wallet.unionkeyLite.UnionKeyLiteCard
import so.unionkey.app.wallet.unionkeyLite.entitys.CardState
import so.unionkey.app.wallet.utils.NfcPermissionUtils
import so.unionkey.app.wallet.utils.Utils
...
class OKLiteManager(private val context: ReactApplicationContext) :
                ...
                launch(Dispatchers.IO) {
                    ...
                    try {
                        ...
                        if (!mNFCConnectedChannel.isEmpty) {
                            ...
                            val startRequest = UnionKeyLiteCard.initRequest(isoDep)
                            val dataMap = Arguments.createMap().apply {
                                putInt("code", -1)
    ...
    override fun onHostPause() {
        Utils.getTopActivity()?.let {
            launch(Dispatchers.IO) {
                try {
                    UnionKeyLiteCard.stopNfc(it as FragmentActivity)
    ...
    private val mNfcStateBroadcastReceiver by lazy {
        object : NfcStatusChangeBroadcastReceiver() {
            ...
            override fun onNfcOn() {
                ...
                Utils.getTopActivity()?.let {
                    launch(Dispatchers.IO) {
                        UnionKeyLiteCard.startNfc(it as FragmentActivity) {}
                    }
                }
            }
    ...
    private suspend fun acquireDevice(): IsoDep? {
        ...
        if (receiveIsoDep == null) {
            ...
            val initChannelRequest = UnionKeyLiteCard.initRequest(receiveIsoDep)
    ...
    fun getCardName(callback: Callback) = launch {
        ...
        handleOperation(callback) { isoDep ->
            ...
            val cardName = UnionKeyLiteCard.getCardName(isoDep)
    ...
    fun getLiteInfo(callback: Callback) = launch {
        ...
        handleOperation(callback) { isoDep ->
            ...
            val cardInfo = UnionKeyLiteCard.getCardInfo(isoDep)
    ...
    fun setMnemonic(mnemonic: String, pwd: String, overwrite: Boolean, callback: Callback) =
        launch {
            handleOperation(callback) { isoDep ->
                ...
                val isSuccess =
                    UnionKeyLiteCard.setMnemonic(mCurrentCardState, isoDep, mnemonic, pwd, overwrite)
    ...
    fun getMnemonicWithPin(pwd: String, callback: Callback) = launch {
        ...
        handleOperation(callback) { isoDep ->
            ...
            UnionKeyLiteCard.getMnemonicWithPin(mCurrentCardState, isoDep, pwd)
        }
    }
    ...
    fun changePin(oldPwd: String, newPwd: String, callback: Callback) = launch {
        ...
        handleOperation(callback) { isoDep ->
            ...
            UnionKeyLiteCard.changPin(mCurrentCardState, isoDep, oldPwd, newPwd)
        }
    }
    ...
    fun reset(callback: Callback) = launch {
        ...
        handleOperation(callback) { isoDep ->
            ...
            val isSuccess = UnionKeyLiteCard.reset(isoDep)
            if (!isSuccess) throw NFCExceptions.ExecFailureException()
