...
import type { IHardwareUiState } from '@unionkey/kit-bg/src/states/jotai/atoms';
// eslint-disable-next-line @typescript-eslint/no-restricted-imports
import { EHardwareUiStateAction } from '@unionkey/kit-bg/src/states/jotai/atoms';
...
function getDeviceSerialNoFromFeatures(
  ...
  return (
    features?.unionkey_serial_no ?? features?.unionkey_serial ?? features?.serial_no
  );
}
...
async function getDeviceVersion(params: IGetDeviceVersionParams): Promise<{
  ...
  const firmwareVersion = usedFeatures
    ? (getDeviceFirmwareVersion(usedFeatures) || []).join('.') ||
      ...
      usedFeatures?.unionkey_firmware_version ||
      ''
    : '';
...
async function getDeviceModeFromFeatures({
  ...
  // https://github.com/UnionKeyHQ/hardware-js-sdk/blob/unionkey/packages/core/src/device/Device.ts#L503
...
async function getDeviceVerifyVersionsFromFeatures({
  ...
  const {
    unionkey_firmware_version: unionkeyFirmwareVersion,
    unionkey_ble_version: unionkeyBleVersion,
    unionkey_boot_version: unionkeyBootVersion,
  } = features;
...
export function parseLocalDeviceVersions({
  ...
  unionkeyFeatures: UnionkeyFeatures;
}): IAllDeviceVerifyVersions {
  return {
    firmware: formatVersionWithHash({
      version: unionkeyFeatures.unionkey_firmware_version,
      checksum: unionkeyFeatures.unionkey_firmware_hash,
      commitId: unionkeyFeatures.unionkey_firmware_build_id,
    }),
    bluetooth: formatVersionWithHash({
      version: unionkeyFeatures.unionkey_ble_version,
      checksum: unionkeyFeatures.unionkey_ble_hash,
      commitId: unionkeyFeatures.unionkey_ble_build_id,
    }),
    bootloader: formatVersionWithHash({
      version: unionkeyFeatures.unionkey_boot_version,
      checksum: unionkeyFeatures.unionkey_boot_hash,
      commitId: unionkeyFeatures.unionkey_boot_build_id,
    }),
  };
}
