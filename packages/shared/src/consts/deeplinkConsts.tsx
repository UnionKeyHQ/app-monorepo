export const UNIONKEY_APP_DEEP_LINK_NAME = 'unionkey-wallet';
export const UNIONKEY_APP_DEEP_LINK = `${UNIONKEY_APP_DEEP_LINK_NAME}://`; // onekey:// will open onekey legacy
export const WALLET_CONNECT_DEEP_LINK_NAME = 'wc';
export const WALLET_CONNECT_DEEP_LINK = `${WALLET_CONNECT_DEEP_LINK_NAME}://`;

export enum EUnionKeyDeepLinkPath {
  url_account = 'url_account',
  market_detail = 'market_detail',
  invite_share = 'invite_share',
}
export type IEUnionKeyDeepLinkParams = {
  [EUnionKeyDeepLinkPath.url_account]: {
    networkCode: string;
    address: string;
  };
  [EUnionKeyDeepLinkPath.market_detail]: {
    coinGeckoId: string;
  };
  [EUnionKeyDeepLinkPath.invite_share]: {
    utm_source: string;
    code: string;
  };
};

// https://explorer-api.walletconnect.com/v3/all?projectId=2f05ae7f1116030fde2d36508f472bfb&entries=40&page=1&search=unionkey&build=1710747625972
export const UNIONKEY_UNIVERSAL_LINK_HOST = 'unionkey.io';
export const UNIONKEY_UNIVERSAL_TEST_LINK_HOST = 'app.api.unionkey.io';

export const WalletConnectUniversalLinkPath = 'wc/connect/wc';
export const WalletConnectUniversalLinkPathSchema = `/wc/connect/wc`; // do not add ? at the end (which meaning optional)
// use /wc/connect but not /wc/connect/wc, the last /wc will be added by WalletConnect SDK
export const WalletConnectUniversalLinkFull = `https://${UNIONKEY_UNIVERSAL_LINK_HOST}/wc/connect`;
