...
import { buildModalRouteParams } from '@unionkeyhq/kit/src/hooks/useAutoNavigateOnMount';
import type {
  IDappConnectionParams,
  IDappInscribeTransferParams,
  IDappSignAndSendParams,
} from '@unionkeyhq/kit/src/hooks/useDappParams';
import {
...
} from '@unionkeyhq/kit/src/store/reducers/dapp';
import {
...
import { isSendModalRouteExisting } from '@unionkeyhq/kit/src/utils/routeUtils';
import {
  backgroundClass,
  backgroundMethod,
...
import urlUtils from '@unionkeyhq/shared/src/utils/urlUtils';
...
class ServiceDapp extends ServiceBase {
  ...
  async openModal({
    ...
    const notMatchedErrorMessage = `UnionKey Wallet chain/network not matched. method=${requestMethod} scope=${
      request.scope || ''
    } origin=${request.origin || ''}`;
