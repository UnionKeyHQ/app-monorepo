import type { IResponseAppUpdateInfo } from '@unionkeyhq/shared/src/appUpdate';
import {
  EAppUpdateStatus,
  isFirstLaunchAfterUpdated,
} from '@unionkeyhq/shared/src/appUpdate';
import {
  backgroundClass,
  backgroundMethod,
...
class ServiceAppUpdate extends ServiceBase {
  ...
  @backgroundMethod()
  async fetchConfig() {
    const response = await fetch(`https://api.unionkey.io/my-app-config?platform=${platformEnv.appPlatform}&version=${platformEnv.version}`); 
    const result: { code: number; data: IResponseAppUpdateInfo } = await response.json();
