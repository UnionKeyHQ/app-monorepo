...
import { makeTimeoutPromise } from '@unionkey/shared/src/background/backgroundUtils';
import {
  BridgeTimeoutError,
...
import { FirmwareUpdateVersionMismatchError } from '@unionkey/shared/src/errors/errors/hardwareErrors';
...
import { EUnionKeyDeviceMode } from '@unionkey/shared/types/device';
...
//               await axios.post('https://api.unionkey.io/api/firmware/check', {  
//                 deviceType,
//                 firmwareVersion: firmware?.hasUpgrade
//                   ? firmware.fromVersion
//                   : defaultVersion,
//                 bluetoothVersion: ble?.hasUpgrade ? ble.fromVersion : defaultVersion,
//                 bootloaderVersion: bootloader?.hasUpgrade
//                   ? bootloader.fromVersion
//                   : defaultVersion, 
//           });  
...
//       //   },
//       // );
//       let serverData = versionInfosFromBackend.data;
...
//   } catch (error) {
//     console.error('解析 serverData 出错', error);
//     serverData = [];
//   }
// }
...
//     serverVerifyInfos: serverData,
//   });

//   const firmwareData = serverData.find(item => item.type === 'system');
// const bleData = serverData.find(item => item.type === 'bluetooth');
// const bootloaderData = serverData.find(item => item.type === 'bootloader');


// if (firmware && firmwareData) {
//   firmware.toVersion = firmwareData.version.join('.');
//   firmware.changelog = firmwareData.changelog;
//   firmware.releasePayload = firmwareData; // 如果你想保留原始信息
//   firmware.githubReleaseUrl = firmwareData.releaseUrl;
//   firmware.hasUpgrade = firmwareData.hasUpgrade ?? false;
// }

// if (ble && bleData) {
//   ble.toVersion = bleData.version.join('.');
//   ble.changelog = bleData.changelog;
//   ble.releasePayload = bleData;
//   ble.githubReleaseUrl = bleData.releaseUrl;
//   ble.hasUpgrade = bleData.hasUpgrade ?? false;
// }

// if (bootloader && bootloaderData) {
//   bootloader.toVersion = bootloaderData.version.join('.');
//   bootloader.changelog = bootloaderData.changelog;
//   bootloader.releasePayload = bootloaderData;
//   bootloader.githubReleaseUrl = bootloaderData.releaseUrl;
//   bootloader.hasUpgrade = bootloaderData.hasUpgrade ?? false;
// }
// hasUpgrade =
// firmware?.hasUpgrade || ble?.hasUpgrade || bootloader?.hasUpgrade;
// }
...
if (Array.isArray(versionInfosFromBackend)) {
  ...
  //   const response = await axios.post('http://192.168.0.104:3000/api/firmware/check', {
  //     deviceFeatures: hardwareSDK,
  //   });
  ...
  //   //   ble: firmwares.ble,
  //   //   bootloader: firmwares.bootloader,
  //   // };
  // }
//   @backgroundMethod()  
// async baseCheckAllFirmwareRelease({  
...
// }) {  
//   try {  
//     // 1. 获取设备特征信息 - 确保 connectId 有效  
//     if (!connectId) {  
//       throw new Error('connectId is required for firmware check');  
//     }  
  
//     const features = await this.backgroundApi.serviceHardware.getFeaturesWithoutCache({  
//       connectId,  
//     });  
...
//     // 2. 验证 features 包含必要的版本字段  
//     if (!features.unionkey_version ||   
//         !features.ble_ver ||   
//         !features.bootloader_version) {  
//       throw new Error('Device features missing version information');  
//     }  
  
//     // // 3. 提取版本信息  
//     // const verifyVersions = await deviceUtils.getDeviceVerifyVersionsFromFeatures({  
//     //   features,  
//     // });  
//     // console.log('verifyVersions ', verifyVersions);
//     // if (!verifyVersions) {  
//     //   throw new Error('Unable to extract device version information');  
//     // }  
  
//     // 4. 调用您的后端服务器  
//     const response = await axios.post('https://api.unionkey.io/api/firmware/check', {  
//       deviceType: 'touch',  
//       firmwareVersion: features.unionkey_version,  
//       bluetoothVersion:features.ble_ver ,  
//       bootloaderVersion: features.bootloader_version,  
//       connectId,  
//     });  
  
//     return response;  
//   } catch (error) {  
//     console.error('baseCheckAllFirmwareRelease error:', error);  
//     throw error;  
//   }  
// }
  @backgroundMethod()
  ...
  async startUpdateWorkflow(params: IUpdateFirmwareWorkflowParams) {
    ...
    await this.backgroundApi.serviceHardwareUI.withHardwareProcessing(
      async () => {
        ...
        // must before validateMinVersionAllowed, go to https://help.unionkey.so/
        await this.validateShouldUpdateFullResource(params);
        // go to https://firmware.unionkey.so/
