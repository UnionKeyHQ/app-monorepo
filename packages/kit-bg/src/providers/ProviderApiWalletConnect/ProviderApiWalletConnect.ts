...
} from '@unionkeyhq/kit/src/components/WalletConnect/WalletConnectClient';
import type { ISessionStatusPro } from '@unionkeyhq/kit/src/components/WalletConnect/WalletConnectClientForDapp';
import { WalletConnectClientForWallet } from '@unionkeyhq/kit/src/components/WalletConnect/WalletConnectClientForWallet';
import {
  backgroundShowToast,
  ...
  handleSessionRequestV2 = async (request: Web3WalletTypes.SessionRequest) => {
    ...
    try {
      ...
      if (!walletConnectUtils.isEvmNamespaceV2(peerNamespace)) {
        throw new Error(
          `UnionKey WalletConnect peer namespace support EVM chain only`,
        );
      }
      if (!peerChainId) {
        throw new Error(
      ...
      if (String(currentChainId) !== peerChainId) {
        ...
        throw new Error(
          `Calling [${params?.request?.method}] ERROR:  Mismatched networks. Please switch UnionKey Wallet current network to chainId=${peerChainId}`,
        );
      }
