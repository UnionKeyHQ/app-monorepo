...
import { UNIONKEY_API_BASE_URL } from '@unionkeyhq/shared/src/config/appConfig';
...
export async function getReleaseInfo(): Promise<PackagesInfo | null> {
  ...
  return axios
    .get<AppReleases>(`${UNIONKEY_API_BASE_URL}/config.json?nocache=${key}`)
...
export async function getChangeLog(
  ...
  let changelogUrl = `${UNIONKEY_API_BASE_URL}/config.json`;
  if (isPreRelease) {
    changelogUrl = `${UNIONKEY_API_BASE_URL}/pre-config.json`;
  }
