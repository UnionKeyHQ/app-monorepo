...
} from '@unionkeyhq/kit/src/store/reducers/autoUpdater';
import {
  ...
  openAppUpdate(versionInfo: VersionInfo): void {
    switch (versionInfo.package.channel) {
      case 'AppStore':
        ...
        break;
      case 'GooglePlay':
        canOpenURL('market://details?id=so.unionkey.app.wallet').then(
          (supported) => {
            if (supported) {
              LinkingOpenURL('market://details?id=so.unionkey.app.wallet');
            } else {
      ...
      case 'HuaweiAppGallery':
        canOpenURL('hiapplink://com.huawei.appmarket?appId=C107439249')
          .then((supported) => {
            ...
            return canOpenURL('market://details?id=so.unionkey.app.wallet');
          })
          .then((supported) => {
