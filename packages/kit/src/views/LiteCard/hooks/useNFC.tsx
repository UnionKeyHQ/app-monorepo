  ...
  const handlerNFCConnectStatus = useCallback(
    ({ code }: { code: number }) => {
      ...
      willCloseDialogInstance.current = Dialog.show({
        ...
        renderContent: (
          <LottieView
            source={require('@unionkeyhq/kit/assets/animations/connect_unionkeylite_connecting.json')}
            height={225}
          />
        ),
      });
    ...
    (callback: () => void) => async () => {
      if (platformEnv.isNativeAndroid) {
        willCloseDialogInstance.current = Dialog.show({
          ...
          renderContent: (
            <LottieView
              source={require('@unionkeyhq/kit/assets/animations/connect_unionkeylite_searching.json')}
              height={205}
            />
          ),
        });
