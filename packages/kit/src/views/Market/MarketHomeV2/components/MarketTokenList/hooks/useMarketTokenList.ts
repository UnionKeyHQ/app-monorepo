...
import type { IMarketTokenListItem } from '@unionkey/shared/types/marketV2';
...
function transformApiDataToComponentData(
  ...
  return apiData.map((item, index) => ({
    ...
    liquidity: parseFloat(item.tvl || '0'), // 使用 TVL 作为流动�?
    transactions: parseInt(item.trade24hCount || '0', 10),
    uniqueTraders: Math.floor(parseInt(item.trade24hCount || '0', 10) / 2), // 估算独特交易者数�?
    holders: item.holders || 0,
    turnover: parseFloat(item.volume24h || '0'),
    tokenAge: '0Y', // API 中没有这个字段，暂时使用默认�?
    audit: 'unknown' as IRiskIndicatorType, // API 中没有这个字段，暂时使用默认�?
