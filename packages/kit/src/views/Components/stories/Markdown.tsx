import { Box, Center, Markdown, ScrollView } from '@unionkeyhq/components';

const MarkdownGallery = () => {
  const md = `
  # UnionKey App Monorepo [English]
  ...
  When you need to install a dependency for a subdirectory, just use \`yarn workspace @unionkeyhq/web add axios\`. With a prefix like \`yarn workspace @unionkeyhq/web\`, the axios module can eventually be installed in the root directory in the web subproject.
  ...
  当需要给某一个子目录安装依赖时，只需要使用 \`yarn workspace @unionkeyhq/web add axios\`。通过 \`yarn workspace @unionkeyhq/web\` 这样的前缀，最终可以在根目录把 axios 模块安装到 web 这个子项目当中。
