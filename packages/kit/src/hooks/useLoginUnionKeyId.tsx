...
const PrimeLoginEmailDialogV2 = LazyLoadPage(
  () =>
    import(
      '@unionkeyhq/kit/src/views/Prime/components/PrimeLoginEmailDialogV2/PrimeLoginEmailDialogV2'
    ),
  0,
  true,
  <Stack>
...
export const useLoginUnionKeyId = () => {
  const intl = useIntl();
  ...
  const toUnionKeyIdPage = useCallback(() => {
    navigation.pushModal(EModalRoutes.ReferFriendsModal, {
      screen: EModalReferFriendsRoutes.UnionKeyId,
    });
  }, [navigation]);

  const loginUnionKeyId = useCallback(
    async ({
      toUnionKeyIdPageOnLoginSuccess,
    }: {
      toUnionKeyIdPageOnLoginSuccess?: boolean;
    } = {}) => {
      const isLoggedIn = await backgroundApiProxy.servicePrime.isLoggedIn();
      const onLoginSuccess = async () => {
        if (toUnionKeyIdPageOnLoginSuccess) {
          await timerUtils.wait(120);
          toUnionKeyIdPage();
        }
      };
    ...
    [intl, logout, toUnionKeyIdPage],
  );
  return useMemo(
    () => ({ sendEmailOTP, loginUnionKeyId }),
    [loginUnionKeyId, sendEmailOTP],
  );
};
