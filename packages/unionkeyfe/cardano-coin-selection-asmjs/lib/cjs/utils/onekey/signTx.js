...
const signTransaction = async (txBodyHex, address, accountIndex, utxos, xprv, signOnly, partialSign) => {
    ...
    const hash = CardanoWasm.FixedTransaction.new_from_body_bytes(transaction.body().to_bytes())
        .transaction_hash()
        .to_hex();
    return {
        signedTx,
...
const signTx = async (keys, tx, keyHashes, partialSign = false) => {
    ...
    const txHash = CardanoWasm.FixedTransaction.new_from_body_bytes(rawTx.body().to_bytes()).transaction_hash();
