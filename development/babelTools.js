...
function normalizeConfig({ platform, config }) {
  ...
  const customAliasForComponents = (name, file) => {
    ...
    if (name.startsWith('use')) {
      return `@unionkeyhq/components/src/Provider/hooks/${name}`;
    }
    return `@unionkeyhq/components/src/${name}`;
  };
  ...
  config.plugins = [
    ...
    [
      'babel-plugin-import',
      {
        'libraryName': '@unionkeyhq/components',
        'camel2DashComponentName': false, // default: true
        'customName': customAliasForComponents,
      },
      '@unionkeyhq_components',
    ],
    [
      'babel-plugin-import',
      {
        'libraryName': '@unionkeyhq/components/src',
        'camel2DashComponentName': false, // default: true
        'customName': customAliasForComponents,
      },
      '@unionkeyhq_components_src',
    ],
    [
