...
const devUtils = require('@unionkeyhq/ext/development/devUtils');
  ...
  apply(compiler) {
    compiler.hooks.done.tap(
      ...
      (compilation, callback) => {
        const msg = `UnionKey Build at ${new Date().toLocaleTimeString()}`;
        setTimeout(() => {
...
function normalizeConfig({
  ...
  config.resolve.fallback = {
    ...
    'crypto': require.resolve(
      '@unionkeyhq/shared/src/modules3rdParty/cross-crypto/index.js',
    ),
